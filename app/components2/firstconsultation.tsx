  ...
  return (
    <section id="primeira-consulta" className="py-28 md:py-36 bg-transparent relative z-10 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* 4 Visual Steps (Alternating Layouts) */}
          ...
          {steps.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className="w-full"
              >
                <div className="p-6 sm:p-10 rounded-[2.5rem] bg-gradient-to-b from-white/[0.06] via-white/[0.02] to-transparent border border-white/10 hover:border-primary/40 backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] transition-all duration-700 group relative overflow-hidden">
                  {/* Inner ambient glow */}
                  ...
                  <div className={`flex flex-col ${isEven ? "lg:flex-row" : "lg:flex-row-reverse"} items-center gap-8 lg:gap-14 relative z-10`}>
                    {/* Image Container */}
                    <div className="w-full lg:w-1/2 overflow-hidden rounded-[2rem] aspect-[16/11] relative shadow-[0_20px_40px_rgba(0,0,0,0.6)] border border-white/10">
                      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent z-10 pointer-events-none"></div>
                      <Image
                        src={item.image}
                        alt={item.title}
                        fill
                        className="object-cover filter contrast-[1.05] grayscale-[20%] group-hover:grayscale-0 group-hover:scale-105 transition-all duration-1000"
                        sizes="(max-width: 1024px) 100vw, 50vw"
                      />
                      <div className="absolute bottom-4 left-4 right-4 z-20">
                        <span className="inline-block px-3.5 py-1.5 rounded-full bg-black/70 backdrop-blur-md border border-primary/30 text-xs font-mont text-primary/90 font-medium tracking-wide">
                          {item.tag}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="w-full lg:w-1/2 flex flex-col justify-center space-y-6">
                      <div className="flex items-center gap-4">
                        <span className="text-4xl sm:text-5xl lg:text-6xl font-mont font-bold text-primary/30 tracking-tighter">
                          {item.step}
                        </span>
                        <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-primary group-hover:border-primary/50 group-hover:bg-primary/10 transition-all duration-500 shadow-md">
                          <Icon className="w-6 h-6" />
                        </div>
                      </div>

                      <h3 className="text-2xl sm:text-3xl lg:text-4xl font-fair text-white group-hover:text-[#FFF099] transition-colors duration-500 leading-snug">
                        {item.title}
                      </h3>

                      <p className="text-base sm:text-lg lg:text-xl font-lora text-white/70 leading-relaxed font-light">
                        {item.description}
                      </p>

                      <div className="pt-2 border-t border-white/10 flex items-center gap-3 text-xs sm:text-sm font-mont text-white/50">
        ...
        {/* Central Statement After 4 Steps & CTA */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 1 }}
          className="text-center mt-24 md:mt-32 space-y-8"
        >
          <div className="w-24 h-[1px] bg-gradient-to-r from-transparent via-primary/50 to-transparent mx-auto"></div>
          
          <h3 className="text-3xl sm:text-4xl md:text-5xl font-fair italic text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] via-[#FFF099] to-[#D4AF37] drop-shadow-[0_0_20px_rgba(212,175,55,0.3)]">
